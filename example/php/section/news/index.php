...
elseif($action=="save") {
        if(isnewsadmin($userID)){
                ...
                if($save) echo'<meta http-equiv="refresh" content="1; url=?site=news">';
                if($preview) echo '<meta http-equiv="refresh" content="1; url=?site=news&action=preview&newsID=$newsID">';
        }
        else redirect('index.php?site=news', 'No access!');
...
elseif($action=="delete") {
        if(isnewsadmin($userID)){
                ...
                else echo '<meta http-equiv="refresh" content="1; url=?site=news">';
        }
        else redirect('index.php?site=news', 'No access!');
}
elseif($action=="edit") {
        ...
        if(isnewsadmin($userID)){
            ...
            while($dr=mysql_fetch_array($newsrubrics)) {
                ...
                if($ds[window1]) $window1='<input class="input" name="window1" type="radio" value="1" checked> Blank <input class="input" type="radio" name="window1" value="0"> Self';
                else $window1='<input class="input" name="window1" type="radio" value="1"> Blank <input class="input" type="radio" name="window1" value="0" checked> Self';

                if($ds[window2]) $window2='<input class="input" name="window2" type="radio" value="1" checked> Blank <input class="input" type="radio" name="window2" value="0"> Self';
                else $window2='<input class="input" name="window2" type="radio" value="1"> Blank <input class="input" type="radio" name="window2" value="0" checked> Self';

                if($ds[window3]) $window3='<input class="input" name="window3" type="radio" value="1" checked> Blank <input class="input" type="radio" name="window3" value="0"> Self';
                else $window3='<input class="input" name="window3" type="radio" value="1"> Blank <input class="input" type="radio" name="window3" value="0" checked> Self';

                if($ds[window4]) $window4='<input class="input" name="window4" type="radio" value="1" checked> Blank <input class="input" type="radio" name="window4" value="0"> Self';
                else $window4='<input class="input" name="window4" type="radio" value="1"> Blank <input class="input" type="radio" name="window4" value="0" checked> Self';
...
elseif($action=="archive") {
        ...
        if($all) {
            if($type=="ASC")
                ...
                if(isnewsadmin($userID)) echo'<form method="post" name="form" action="?site=news">';
                eval ("\$news_archive_head = \"".gettemplate("news_archive_head")."\";");
            echo $news_archive_head;
                $i=1;
                while($ds=mysql_fetch_array($ergebnis)) {
				...
				if(isnewsadmin($userID)) $admdel='<form method="post" name="form" action="?site=news">
												  <table width="100%" border="0" cellspacing="0" cellpadding="4">
                                                        <tr>
                                                        	<td><input class="input" type="checkbox" name="ALL" value="ALL" onClick="SelectAll(this.form);">Select all</td>
                                                            <td align="right"><select name="quickactiontype">
		...
		if(isnewsadmin($userID)) { 
                ...
                if($unpublished) $publish='<a href="?site=news&action=unpublished">'.$unpublished.' unpublished News</a>';
    ...
    $i=1;
        while($ds=mysql_fetch_array($result)) {
            if($i%2) $bg1=BG_1;
                ...
                $content = toggle($content, $ds[newsID]);
                $poster=''.getnickname($ds[poster]).'';
                if($ds[link1] && $ds[url1]!="http://" && $ds[window1]) $related.='&#8226; <a href="'.$ds[url1].'" target="_blank">'.$ds[link1].'</a><br> ';
                if($ds[link1] && $ds[url1]!="http://" && !$ds[window1]) $related.='&#8226; <a href="'.$ds[url1].'">'.$ds[link1].'</a><br> ';

                if($ds[link2] && $ds[url2]!="http://" && $ds[window2]) $related.='&#8226; <a href="'.$ds[url2].'" target="_blank">'.$ds[link2].'</a><br> ';
                if($ds[link2] && $ds[url2]!="http://" && !$ds[window2]) $related.='&#8226; <a href="'.$ds[url2].'">'.$ds[link2].'</a><br> ';

                if($ds[link3] && $ds[url3]!="http://" && $ds[window3]) $related.='&#8226; <a href="'.$ds[url3].'" target="_blank">'.$ds[link3].'</a><br> ';
                if($ds[link3] && $ds[url3]!="http://" && !$ds[window3]) $related.='&#8226; <a href="'.$ds[url3].'">'.$ds[link3].'</a><br> ';
                ...
                if($ds[comments]) {
                        if($ds[cwID]) {
                                ...
                                if($anzcomments=="1") $comments='<a href="index.php?site=clanwars_details&cwID='.$ds[cwID].'">['.$anzcomments.'] Kommentar</a> <br>letzter '.getlastcommentposter($ds[cwID], 'cw').', am '.date("d.m.y, H:i", getlastcommentdate($ds[cwID], 'cw'));
				elseif($anzcomments>"1") $comments='<a href="index.php?site=clanwars_details&cwID='.$ds[cwID].'">['.$anzcomments.'] Kommentare</a> <br>letzter '.getlastcommentposter($ds[cwID], 'cw').', am '.date("d.m.y, H:i", getlastcommentdate($ds[cwID], 'cw'));
                                else $comments='<a href="index.php?site=clanwars_details&cwID='.$ds[cwID].'">Keine Kommentare</a>';
                        }
                        else {
                                $anzcomments = getanzcomments($ds[newsID], 'ne');
                                if($anzcomments=="1") $comments='<a href="index.php?site=news_comments&newsID='.$ds[newsID].'">['.$anzcomments.'] Kommentar</a> <br>letzter '.getlastcommentposter($ds[newsID], 'ne').', am '.date("d.m.y, H:i", getlastcommentdate($ds[newsID], 'ne'));
				elseif($anzcomments>"1") $comments='<a href="index.php?site=news_comments&newsID='.$ds[newsID].'">['.$anzcomments.'] Kommentare</a> <br>letzter '.getlastcommentposter($ds[newsID], 'ne').', am '.date("d.m.y, H:i", getlastcommentdate($ds[newsID], 'ne'));
                                else $comments='<a href="index.php?site=news_comments&newsID='.$ds[newsID].'">Keine Kommentare</a>';
                        }
                }
                else $comments='';

                if(isnewsadmin($userID)) $adminaction='<a href="?site=news&action=edit&newsID='.$ds[newsID].'">Edit</a>
